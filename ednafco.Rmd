...
datafile <- "data/eDNA_FCM_data_20240215.xlsx"
...
ggsave("figures/fig2_taxa_monitored_CCB_GS_eDNAterr.png", dpi = 400, width = 6, height = 4)
```
...
b %>%
  ...
  geom_boxplot(aes(x = as.factor(nummethods), y = numtaxgroups, 
    ...
    geom_point(aes(x = jitter(as.numeric(as.factor(nummethods)), 1.3), y = numtaxgroups,
                 color = type), 
             shape = 21, alpha = 0.75, size = 3,
             data = filter(b, type == "FCO projects", nummethods==1, numtaxgroups>0)) +
...
ggsave("figures/fig_taxa_monitored_v_methods_used_20240216.png", width = 3, height = 3, dpi =400)
```
...
summary(p[p$numanimalgroups>0,]$numanimalgroups)
hist(p[p$numanimalgroups>0,]$numanimalgroups)
summary(p[p$numtaxgroups>0,]$numtaxgroups)
hist(p[p$numtaxgroups>0,]$numtaxgroups)
summary(p[p$nummethods>0,]$nummethods)
hist(p[p$nummethods>0,]$nummethods)
length(p[p$nummethods>0,]$nummethods)
...
ggsave("figures/fig_map_projects4b.png", width = 6, height = 6, dpi = 400)

# summarize number of projects at different latitudes
sum(abs(as.numeric(pall$lat_approx))<23.5, na.rm = T) # 40
sum(abs(as.numeric(pall$lat_approx))>23.5, na.rm = T) # 38
sum(abs(as.numeric(pall$lat_approx))<35, na.rm = T) # 63
sum(abs(as.numeric(pall$lat_approx))>35, na.rm = T) # 15
...
ggsave("figures/fig_map_eDNAstudies2.png", width = 6, height = 6, dpi =400)
...
ggsave("figures/fig1_methods_fco_20240216.png", width = 7, height = 5, dpi =600)
...
ggsave("figures/fig2_methods_fco_by_org_20240216d.png", width = 9, height = 4, dpi =600)
