...
ggsave("figures/fig2_taxa_monitored2_full.png", dpi = 400, width = 6, height = 4)
```
...
sum(abs(p$lat_approx)<23.5, na.rm = T) # 32
sum(abs(p$lat_approx)>23.5, na.rm = T) # 24
sum(abs(p$lat_approx)<35, na.rm = T) # 48
sum(abs(p$lat_approx)>35, na.rm = T) # 8

sum(!is.na(p$lat_approx)) # 56
48/56
8/56
```
...
# summarize number of projects at different latitudes
sum(abs(e1$lat_approx)<23.5, na.rm = T) # 17
